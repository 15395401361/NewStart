...
- (void)show {
    ...
    __block UITableView *weakTab = _tableView;
    WS(weakSelf)
    [UIView animateWithDuration:0.2 animations:^{
        weakSelf.alpha = 1;
        weakTab.transform = CGAffineTransformMakeScale(1.0, 1.0);
    }];
}
...
- (void)hide {
    ...
    [UIView animateWithDuration:0.2 animations:^{
        weakSelf.alpha = 0;
        weakTab.transform = CGAffineTransformMakeScale(0.0001, 0.0001);
    } completion:^(BOOL finished) {
        [weakTab removeFromSuperview];
        [weakSelf removeFromSuperview];
        if (weakSelf.hideHandle) {
            weakSelf.hideHandle();
        }
    }];
}
